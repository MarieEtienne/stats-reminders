...
For practicality, or for some EDO models that are not explicitly solvable (i. e. we can't find a unique solution to the Cauchy problem), we can locally and numerically solve in the same time we calculate the likelihood. This alternative is used in the next part.
...
```{r}
logistic=function(t,N,parametre){
  r=parametre[1]
  K=parametre[2]
  dNdt = r*(1-N/K)*N
  list(c(dNdt))
...
MMV <- function(theta){
  ...
  K1 <- theta[2]
  N0 <- theta[3]
  sol = ode(y = N0, times = time, func = logistic, parms = c(r1, K1))
  mod_pred <- sol[, 2]
