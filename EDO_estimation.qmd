...
A biological example of this is *Paramecium aurelia*, a paramecia species that exhibits logistic growth when maintained in isolated populations.
...
# Store in a dataframe for the plot
data <- data.frame(time = time, sim_data = sim_data)

# Plot the result
library(ggplot2)

ggplot(data,aes(x = time, y = sim_data)) +
  geom_point(shape = 1, size = 2) +
  labs(
    title = "Logistic Growth with random noise",
    x = "Time",
    y = "Population size") +
  theme_minimal() +
...
```{r, echo=FALSE, fig.align = 'center'}
# Store in a dataframe for the plot
data_and_models <- data.frame(time = time,
                              sim_data = sim_data,
                              mod_pred_LS = mod_pred_LS,
                              mod_pred_ML = mod_pred_ML)

# plot
ggplot(data_and_models, aes(x = time)) +
  geom_point(aes(y = sim_data, color = "Simulated data"), shape = 1, size = 2) + 
  geom_line(aes(y = mod_pred_LS, color = "Least Squares"), linewidth = 0.8) +
  geom_line(aes(y = mod_pred_ML, color = "Maximum Likelihood"), linetype = 'dashed', linewidth = 0.8) +
    scale_color_manual(
    values = c(
      "Simulated data" = "black",
      "Least Squares" = "blue",
      "Maximum Likelihood" = "red"
      )
    ) +
  labs(title = "Logistic Growth with random noise and fitted models",
       x = "Time",
       y = "Population size") +
  theme_minimal() +
  theme(
