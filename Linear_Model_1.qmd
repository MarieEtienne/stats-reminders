...
The concepts covered in this reminder sheet are taken from our Master's lectures, written by @Y_Outreman and then taught by @A_Masson . During these courses, we learned how to explore data and then analyze it using this type of model.
:::
...
```{r data_importation, echo=TRUE, message = FALSE, results='markup'}
# Data import 
...
```{r process_NA_values, echo=TRUE, results='markup'}
# Check for presence of missing values
...
::: {#fig-distrib-bodymass}
```{r data_response_variable, echo=TRUE, results='markup'}
par(mfrow=c(2,2),      
    mar=c(4,4,2,1),           
    bg="gray95",              
    col.axis="gray20",      
...
::: {#fig-quantitative-explanatory-var}
```{r data_quantitative_explanatory_variables, echo=TRUE, results='markup'}
#Don't show the code 
par(mfrow=c(2,2),      
    mar=c(4,4,2,1),           
    bg="gray95",              
...
```{r data_cross_qualitative_exploration, echo=TRUE, results='markup'}
...
```{r data_interaction_qualitative, echo=TRUE, results='markup'}
...
::: {#fig-interactions}
```{r data_interaction_qualitative_quantitative, echo=TRUE, results='markup'}
par(mfrow=c(3,2)) 
...
Here are our observations regarding possible interactions, based on the graphs of @fig-interactions :
...
Collinearity should be avoided in modelling. Indeed, it can affect the modelling and the significance of the variables in the model, which affects the conclusions. Let's check how the explanatory variables are related (@fig-collinearity).
...
::: {#conditionsBACK}
After selection, the [conditions](#conditions) of the linear model must be validated before making interpretations and conclusions.
:::
...
```{r fullmodel, echo=TRUE, results='markup', include=TRUE}
...
```{r normality, echo=TRUE, results='markup'}
par(mfrow=c(1,2))
# Histogram
...
The hypothesis of the homogeneity of the variance means that the variaance of residuals is constant over the entire range of explanatory variables. The residuals must be plotted against the estimated values and against the values of the significant variables. We can validate this hypothesis if the residuals show no trend.

```{r, homogeneity, echo=TRUE, results='markup'}
par(mfrow=c(2,3))
...
These results are consistent with the literature : morphological traits such as flipper length and bill depth are known to scale positively with structural size in penguins, and species-level differences in body mass, particularly the larger size of Gentoo penguins, have been documented (@Williams1995).
