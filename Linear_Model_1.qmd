...
![Adelie Penguins from Petrel Island - Adelie Land - C. Cornec (IPEV/CNRS/CSM)](Adelie_Penguins.jpg)

In this application example, we use data from a study conducted by Dr. Kristen Gorman, who studied the physical characteristics of three species of penguins in the Palmer Archipelago in Antarctica. The data was collected on three islands of the archipelago in 2007, 2008, and 2009.
...
The model can be build like this : model \<- lm (`body_mass` \~ `...`, data = `penguins`)
...
There are NA values in the dataset. We decide to remove them but there are different options for handling the presence of NA values.
...
### **4. Analysis of the Potential Relationships Between \$Y\$ and the \$X\$s**
