...
Another important aspect to consider is that for this type of model to be valid, three conditions must be met: - the model residuals must follow a normal distribution - homoscedasticity (homogeneity of the variance of the residuals) must be respected - the residuals are independent
...
Firstly, we need to import the data, explore it, and see if there are any NA values, outliers, and so on. The data is accessible via the package palmerpenguins by \@Holst2020.
