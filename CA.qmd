...
Species absent from all surveys (empty columns) or present in only one samples were removed to prevent double absences from impacting the analysis.
```{r}
...
#Selection of the species that contribute significantly to the axes one and two
...
```{r}
#| label: fig-CA_eig1
#| fig-cap: "Eigen values associated to CA dimensions"
#| fig-subcap: 
#| - "With Bassin samples"
...
Better explanation without the "Bassin"
...
ggplot(data = CA_ade4_sansbassin_ligne_ZH) +
  ...
  theme(axis.text.x = element_markdown(vjust = 1, hjust = 1, size = 11), 
        ...
        legend.position = "none") 
```
In @fig-CA_CAwetland_position, the points correspond to the projection of the samples onto the two axes of the PCA. The position of each sampling point corresponds to the barycenter (average) of the coordinates of the species present at that site. 
...
ggplot(data = CA_ade4_sansbassin_col_ZH) +
  ...
  theme(axis.text.x = element_markdown(vjust = 1, hjust = 1, size = 11), 
        ...
        legend.position = "none") 
```
In @fig-CA_CAspecies_position, the points correspond to the projection of the families onto the two axes of the PCA. The position of each family corresponds to the barycenter (average) of the coordinates of the sites where this species was present.
...
s.distri(CA_ade4_sansbassin$li, macroinv_red[-c(27,28,29), 26], cell = 1.5, 
         sub = colnames(macroinv_red[26]), csub = 2) #family rare
s.distri(CA_ade4_sansbassin$li, macroinv_red[-c(27,28,29), 36], cell = 1.5, 
         sub = colnames(macroinv_red[36]), csub = 2) #family rare
s.distri(CA_ade4$li, macroinv_red[, 22], cell = 1.5,
         sub = colnames(macroinv_red[22]), csub = 2) #family commune
...
# s.distri(CA_ade4_sansbassin$co, macroinv_red[5, -c(1, 2)],sub = "Site 5", csub = 2)
```
