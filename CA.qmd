...
This table value shows presence and abundance, each square representing the presence of a family in a site. The square size varies with abundance of the family. On the first table (@fig-CA_tablevalues-1) - which includes the 3 bassin stations - we can see that the presence profils are highly impacted by the peculiar macroinvertebrate composition of said stations. 

When we get rid of these stations (@fig-CA_tablevalues-2), it reveals presence patterns that couldn't be seen. The most obvious is the bivalve presence in the first sations, which is likely due to their prevalence in "Ruche" stations. The graph also shows that the presence of *Culicidae* is similar in all stations. For *Physidae*, we observe the same tendency although they are not present on the station "Rieu_Querelle". Some species are only present in a few, or even a single station. That is the case for *Hydrobiidae* which are only present in one sample, "Foot_2". Finally, *Baetidae*'s presence is really high on the "Verdanson" Stations.
