...
macroinv_red %>% 
  ...
  # summarize(div = n()) %>%
  ggplot(., aes(x = Echantillon, fill = ZH)) +  
  geom_bar() + 
  ggtitle("Nombre de familles de macro-invertébrés par zones humides") +
  theme_bw()
```
...
macroinv_red %>% 
  ...
  facet_wrap(~ ZH) + 
    ggtitle("Abondance d'individus par famille de macro-invertébrés pour chaque échantillon par zone humide") +
  theme_bw() + 
...
macroinv_red %>% 
  group_by(ZH) %>% 
  pivot_longer(cols = c(3:40), names_to = "Familles", values_to = "Abondance") %>%
  left_join(., classification, by = c("Familles" = "X2")) %>%
  ggplot(., aes(x = Echantillon, y = Abondance, fill = X1)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ ZH) + 
  ggtitle("Abondance d'individus par famille de macro-invertébrés par réplica pour chaque zone humide") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), 
        plot.title = element_text(hjust = 0.5))
```
