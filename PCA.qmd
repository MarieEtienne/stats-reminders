...
-   rAME : relative Anterior Median Eyes (colored in yellow in @fig-fig1)

-   rALE : relative Anterior Lateral Eyes (colored in red in @fig-fig1)

-   rPME : relative Posterior Median Eyes (colored in blue in @fig-fig1)

-   rPLE : relative Posterior Lateral Eyes (colored in green in @fig-fig1)

```{r}
#| echo: false
#| label: fig-fig1
#| fig-cap: "Typical arrangements of spider eyes and their modification across spider families"

knitr::include_graphics("./arrangements_of_spider_eyes.jpg")
```
...
```{r}
#| label: fig-indiviual_graph_selectind
...
According to @fig-var_contrib, the first axis is positively correlated with the relative size of both the anterior and posterior lateral eyes (the red and green ones in @fig-fig1). For the second axis, there seems to be an antagonistic relationship between the anterior and posterior eyes.
