...
The cos2 indicates the representation of the variables on the dimension. rAME (cos2 = 0.36) and rPME (cos2 = 0.47) are moderately represented on Dim.1 whereas rALE (cos2 = 0.76) and rPLE (cos2 = 0.88) are very well reprensented on this dimension
...
fviz_pca_ind(res.pca,
             select.var = list(name = selected_ind),
             geom.ind = "point",
             label = "none",
             habillage = spiders$Family,
             legend = "none",
             repel = TRUE,
             pointshape = 19)
```
...
Because there is 106 family of species we will concentrate only on the first height most frequent family in order to have a clearer vision.
...
According to @var_contrib, we found that the first axe is positively correlated to the relative size of both anterior and posterior lateral eyes (red and green ones in @fig1). For the second axe, it seems that there is a antagonist relation between anterior and posterior.
